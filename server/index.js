...
const STALE_AGENT_IDS = new Set(['people-officials', 'emperor', 'queen', '司礼监', 'silijian', 'sili-jian']);

function normalizeAgentKey(value) {
  return String(value || '').trim().toLowerCase();
}

function isStaleAgentId(value) {
...
function getAgentDisplayName(agentId, displayName) {
  const normalizedAgentId = normalizeAgentKey(agentId);
  const normalizedDisplayName = normalizeAgentKey(displayName);
  if (normalizedDisplayName && AGENT_DEPT_MAP[normalizedDisplayName]) return AGENT_DEPT_MAP[normalizedDisplayName];
  if (normalizedAgentId && AGENT_DEPT_MAP[normalizedAgentId]) return AGENT_DEPT_MAP[normalizedAgentId];
  if (displayName && !isStaleAgentId(displayName)) return displayName;
  if (!isStaleAgentId(agentId)) return agentId;
  return '历史归档机构';
}

function isVisibleAgent(agentId, displayName) {
...
app.get('/api/status', authMiddleware, async (req, res) => {
  ...
  const botAccounts = agentIds
    .filter(id => isVisibleAgent(id, AGENT_DEPT_MAP[id] || id))
    .map(id => {
      ...
      return {
        name: id,
        displayName: getAgentDisplayName(id, AGENT_DEPT_MAP[id] || id),
        status: 'online',
        model: model,
...
function getTokenStats() {
  ...
  if (existsSync(AGENTS_DIR)) {
    ...
    for (const agentDir of agentDirs) {
      if (!isVisibleAgent(agentDir, AGENT_DEPT_MAP[agentDir] || agentDir)) continue;
      ...
      if (existsSync(sessionsPath)) {
        try {
          ...
          const deptName = getAgentDisplayName(agentDir, deptMap[agentDir] || agentDir);
...
function buildSessionsData() {
  ...
  if (existsSync(AGENTS_DIR)) {
    ...
    for (const agentId of agentDirs) {
      if (!isVisibleAgent(agentId, AGENT_DEPT_MAP[agentId] || agentId)) continue;
      const sessionsPath = join(AGENTS_DIR, agentId, 'sessions', 'sessions.json');
      if (existsSync(sessionsPath)) {
        try {
          const data = JSON.parse(readFileSync(sessionsPath, 'utf-8'));
          for (const [sessionKey, session] of Object.entries(data)) {
            ...
            sessions.push({
              ...
              agentId,
              agentName: getAgentDisplayName(agentId, AGENT_DEPT_MAP[agentId] || agentId),
              channel,
              updatedAt,
...
app.get('/api/departments/:name/recent', authMiddleware, (req, res) => {
  try {
    ...
    for (const [id, name] of Object.entries(AGENT_DEPT_MAP)) {
      if (!isVisibleAgent(id, name)) continue;
...
app.post('/api/command', authMiddleware, async (req, res) => {
  ...
  const targetChannel = channel || '1474091579630293164'; // 默认战情室频道
...
app.get('/api/bots', authMiddleware, (req, res) => {
  try {
    const config = JSON.parse(readFileSync(CONFIG_PATH, 'utf-8'));
    const accounts = config.channels?.discord?.accounts || {};
    const bots = Object.entries(accounts)
      .filter(([id, acc]) => isVisibleAgent(id, acc.displayName || AGENT_DEPT_MAP[id] || id))
      .map(([id, acc]) => ({
        id,
        name: getAgentDisplayName(id, AGENT_DEPT_MAP[id] || id),
        displayName: getAgentDisplayName(id, acc.displayName || AGENT_DEPT_MAP[id] || id),
...
app.get('/api/location/track', authMiddleware, async (req, res) => {
  ...
  const role = req.query.role || 'unknown'; // e.g. president / vice-president
