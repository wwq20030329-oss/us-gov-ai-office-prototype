...
import type { SystemStatus, TabName } from "../types"
import { useTheme } from "../theme"
import { filterVisibleBots, filterVisibleNames, getAgentDisplayName, mapAgentLabel } from "../utils/agentDisplay"
import { PRODUCT_NAME, getOperationalStatus, getToneClasses, getWorkflowStage } from "../utils/officeSemantics"
...
function TrendTooltip({ active, payload, label, dailyTrend }: any) {
  ...
  if (idx > 0 && dailyTrend) {
    ...
    if (prev > 0) {
      ...
      change = `${pct >= 0 ? '↑' : '↓'} ${Math.abs(pct).toFixed(1)}% 较前日`
    }
  }
  ...
  return (
    <div className={`px-3 py-2 rounded-lg border text-xs ${
      ...
      <div className="font-mono mt-1">{fmt(value)} 令牌</div>
  ...
  return (
    <div className={`${theme === 'light' ? 'bg-white border border-gray-200' : 'bg-[#1a1a2e]'} rounded-lg p-3 sm:p-4`}>
      <div className="flex items-center justify-between mb-3">
        <h3 className={`text-[10px] sm:text-xs uppercase tracking-wider ${sub}`}>📈 7日令牌趋势</h3>
        <div className="flex items-center gap-1">
          <span className={`text-xs font-mono ${isUp ? 'text-red-400' : 'text-green-400'}`}>
            {isUp ? '↑' : '↓'} {Math.abs(diff).toFixed(1)}%
          </span>
          <span className={`text-[10px] ${sub}`}>较昨日</span>
        </div>
      </div>
...
export default function Dashboard({ data, onNavigate }: Props) {
  ...
  const onlineCount = visibleBots.filter(b => b.status === "online").length
  const totalCount = visibleBots.length
  const HIGH_TOKEN_ALERT = 2000000

  const sortedByTokens = [...visibleBots].sort((a, b) => b.totalTokens - a.totalTokens)
  const maxTokens = sortedByTokens[0]?.totalTokens || 1

  const realTotalTokens = summary?.totalTokens ?? visibleBots.reduce((sum, bot) => sum + bot.totalTokens, 0)
  const realTotalSessions = summary?.totalSessions ?? visibleBots.reduce((sum, bot) => sum + bot.sessions, 0)
  const realActiveSessions = summary?.activeSessions ?? 0

  const deptRanking = filterVisibleNames(summary?.deptRanking || [], d => d.name).map(d => ({
    ...d,
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <section className="panel-hero px-5 py-5 sm:px-6 sm:py-6">
        <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <div className="page-kicker">Dashboard / Executive Overview</div>
            <h2 className="page-title">{PRODUCT_NAME} 统一运行总览</h2>
            <p className="page-desc">
              现在最丑的是信息层级太散、表情符号太吵、卡片像临时拼上去的。这轮把总览改成统一的控制台语气：先看总体状态，再看风险，再看机构与任务分布。
            </p>
          </div>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3 xl:w-[30rem]">
            {[
              { label: '在线席位', value: `${onlineCount}/${totalCount}` },
              { label: '活跃任务', value: realActiveSessions.toString() },
              { label: '累计令牌', value: fmt(realTotalTokens) },
            ].map((item) => (
              <div key={item.label} className="rounded-2xl border border-white/10 bg-white/6 px-4 py-3 backdrop-blur-sm">
                <div className="text-[11px] tracking-[0.16em] uppercase text-white/60">{item.label}</div>
          ...
          <div className="flex flex-wrap gap-4 justify-around">
            {Object.entries(locations).map(([role, loc]) => (
              <div key={role} className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-xs sm:text-sm font-medium">{role === 'president' ? '总统' : role === 'vice-president' ? '副总统' : role}</div>
      ...
      {/* Token 告警 */}
      {realTotalTokens > HIGH_TOKEN_ALERT && (
        <div className="bg-yellow-500/10 border border-yellow-500/50 rounded-lg p-3 flex items-center gap-2">
          <span>⚠️</span>
          <span className="text-yellow-500 text-xs sm:text-sm">今日令牌消耗已超 2M：{fmt(realTotalTokens)}</span>
        </div>
      )}

      <MainViewHeader
        eyebrow="Executive Operations Overview"
        title={`${PRODUCT_NAME} 运行总览`}
        description="Dashboard 负责统一展示机构席位、办公大厅与会话任务的共享状态。"
        links={[
          { label: '查看办公大厅', tab: 'office' },
          { label: '查看机构席位', tab: 'departments' },
          { label: '查看会话任务', tab: 'sessions' },
        ]}
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        {[
          { label: '执行中席位', value: `${onlineCount}/${totalCount}`, icon: '🏛️' },
          { label: '总令牌', value: fmt(realTotalTokens), icon: '🔥' },
          { label: '会话任务', value: realTotalSessions.toString(), icon: '💬' },
          { label: '执行中任务', value: realActiveSessions.toString(), icon: '⚡' },
        ].map(c => (
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className={`${bg} rounded-lg p-3 text-center`}>
          <div className={`text-[10px] ${sub}`}>📊 1 分钟负载</div>
          <div className={`font-mono text-sm mt-1 ${loadColor(load1m * 25)}`}>{load1m.toFixed(2)}</div>
          <div className={`text-[9px] mt-1 ${sub}`}>系统负载均值，并非 CPU 百分比</div>
        </div>
        <div className={`${bg} rounded-lg p-3 text-center`}>
      ...
      {/* 7日令牌趋势图 - 真实数据 + 增强 tooltip */}
      {summary?.dailyTrend && <TokenTrend dailyTrend={summary.dailyTrend} />}

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
        <div className={`${bg} rounded-lg p-3 sm:p-4 lg:col-span-2`}>
          <h3 className={`text-[10px] sm:text-xs uppercase tracking-wider mb-2 ${sub}`}>🏛️ 信息架构</h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs sm:text-sm">
            {[
              { title: 'Dashboard', desc: '统一总览与跨视图指标' },
              { title: 'Office', desc: '像素办公大厅与席位映射' },
              { title: 'Departments', desc: '机构席位与职能状态' },
              { title: 'Sessions', desc: '会话任务与执行轨迹' },
            ].map((item) => (
              <div key={item.title} className={`rounded-lg p-3 ${theme === 'light' ? 'bg-gray-50' : 'bg-[#0d0d1a]'}`}>
                <div className="font-medium text-[#d4a574]">{item.title}</div>
                <div className={`mt-1 text-[11px] leading-relaxed ${sub}`}>{item.desc}</div>
              </div>
            ))}
          </div>
        </div>
        <div className={`${bg} rounded-lg p-3 sm:p-4`}>
          <h3 className={`text-[10px] sm:text-xs uppercase tracking-wider mb-2 ${sub}`}>🧭 状态字典</h3>
          <div className="space-y-2 text-xs">
            {[
              getOperationalStatus('online', 1),
              getOperationalStatus('online', 0),
              getOperationalStatus('offline', 0),
            ].map((status) => {
              const tone = getToneClasses(status.tone)
              return (
                <div key={status.label} className="flex items-start gap-2">
                  <span className={`mt-1 w-2 h-2 rounded-full ${tone.dot}`} />
                  <div>
                    <div className={`font-medium ${tone.text}`}>{status.label}</div>
                    <div className={sub}>{status.description}</div>
                  </div>
                </div>
              )
            })}
      ...
      {topActive.length > 0 && (
        <div className={`${bg} rounded-lg p-3 sm:p-4`}>
          <h3 className={`text-[10px] sm:text-xs uppercase tracking-wider mb-3 ${sub}`}>🕐 近期活跃机构</h3>
          <div className="space-y-2">
            {topActive.map((dept, i) => {
              const stage = getWorkflowStage(dept.updatedAt, dept.messages)
              return (
                <div key={dept.name} className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-[10px] px-1.5 py-0.5 rounded border border-[var(--border-subtle)] text-[var(--text-secondary)] bg-[var(--bg-soft)]">
                      {stage.label}
                    </span>
      ...
      <div className={`${bg} rounded-lg p-3 sm:p-4`}>
        <h3 className={`text-[10px] sm:text-xs uppercase tracking-wider mb-3 ${sub}`}>🧭 主视图职责分工</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-2 text-xs sm:text-sm">
          {[
            { title: 'Dashboard', desc: '统一观察全局状态与跨页入口', tab: 'dashboard' as TabName },
            { title: 'Office', desc: '把机构状态映射成办公大厅', tab: 'office' as TabName },
            { title: 'Departments', desc: '查看机构席位与职责状态', tab: 'departments' as TabName },
            { title: 'Sessions', desc: '查看会话任务与消息轨迹', tab: 'sessions' as TabName },
          ].map((item) => (
            <button
              key={item.title}
              onClick={() => onNavigate?.(item.tab)}
              className={`rounded-lg p-3 text-left border transition-colors cursor-pointer ${theme === 'light' ? 'bg-gray-50 border-gray-200 hover:bg-gray-100' : 'bg-[#0d0d1a] border-[#d4a574]/10 hover:bg-[#16213e]'}`}
            >
              <div className="font-medium text-[#d4a574]">{item.title}</div>
              <div className={`mt-1 text-[11px] leading-relaxed ${sub}`}>{item.desc}</div>
            </button>
      ...
      {/* 令牌消耗排行 - 可点击跳转会话 */}
      <div className={`${bg} rounded-lg p-3 sm:p-4`}>
        <h3 className={`text-[10px] sm:text-xs tracking-wider mb-3 ${sub}`}>🔥 令牌消耗排行</h3>
          ...
          {sortedByTokens.filter(d => d.totalTokens > 0).map((bot, i) => {
            ...
            return (
              <div
                ...
                onClick={() => handleDeptClick(mapAgentLabel(bot.displayName || bot.name))}
                title={`点击查看${mapAgentLabel(bot.displayName || bot.name)}的会话任务`}
              >
                <span className={`w-4 text-[10px] sm:text-xs font-mono ${i < 3 ? 'text-[#d4a574] font-bold' : sub}`}>{i + 1}</span>
                <span className="w-12 sm:w-16 text-[10px] sm:text-xs truncate">{mapAgentLabel(bot.displayName || bot.name)}</span>
      ...
      {/* 联邦机构状态 */}
      <div>
        <h3 className={`text-[10px] sm:text-xs tracking-wider mb-2 sm:mb-3 ${sub}`}>🏛️ 机构席位状态</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3">
          {visibleBots.map(bot => {
            const visibleName = mapAgentLabel(bot.displayName || bot.name)
            const deptInfo = deptRanking.find(d => d.name === visibleName)
            ...
            return (
              <div
                key={bot.name}
                className={`${bg} rounded-lg p-2.5 sm:p-3 border-l-2 cursor-pointer hover:ring-1 hover:ring-[#d4a574]/30 transition-all ${op.tone === 'online' ? 'border-green-500' : op.tone === 'idle' ? 'border-yellow-500' : 'border-gray-500'}`}
                onClick={() => handleDeptClick(visibleName)}
              >
                <div className="flex items-center justify-between mb-0.5 gap-2">
                  <span className="text-xs sm:text-sm font-medium truncate">{visibleName}</span>
                  <span className={`text-[9px] px-1.5 py-0.5 rounded border ${tone.pill}`}>{op.label}</span>
                </div>
                <div className={`text-[10px] ${sub} truncate`}>{bot.model?.replace(/^[^/]+\//, '') || '未分配模型'}</div>
                <div className="flex justify-between mt-0.5 text-[10px] sm:text-xs">
                  <span className={sub}>任务 {bot.sessions}</span>
