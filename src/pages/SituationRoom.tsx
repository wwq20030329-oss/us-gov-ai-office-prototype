...
const SITUATION_ROOM_CHANNEL = '1474091579630293164'
...
function PixelPerson({ bot, selected, online, onClick, rank }: {
  bot: Bot; selected: boolean; online: boolean; onClick: () => void; rank?: 'executive' | 'agency' | 'support'
}) {
  const c = COLORS[bot.id] || { bg: '#888', hat: '#666' }
  const size = rank === 'executive' ? 'scale-125' : rank === 'agency' ? 'scale-110' : ''
...
export default function SituationRoom() {
  ...
  const fetchChannelMessages = async () => {
    ...
    try {
      const r = await fetch(`/api/channel-messages?channel=${SITUATION_ROOM_CHANNEL}&limit=15`, {
  ...
  const sendCommand = async () => {
    ...
    try {
      const r = await fetch('/api/command', {
        ...
        body: JSON.stringify({ channel: SITUATION_ROOM_CHANNEL, message: finalMessage, botId: target })
      })
      const d = await r.json()
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg overflow-hidden shadow-2xl">
        ...
        <div className="relative">
          <div className="bg-gradient-to-b from-[#8b0000] via-[#6b0000] to-[#4a0000] p-4 text-center border-b-4 border-[#ffd700]/30">
            ...
            <div className="text-[#ffd700]/40 text-xs mt-1 tracking-[0.3em]">WHITE HOUSE COMMAND GRID / 中文优先</div>
          </div>
        </div>
          ...
          <div className="relative p-4 sm:p-6">
            ...
            <div className="text-center mb-4">
              <div className="inline-block px-4 py-0.5 rounded-full bg-[#ffd700]/10 border border-[#ffd700]/20 mb-3">
                <span className="text-[10px] sm:text-xs text-[#ffd700]/70 tracking-wider">🦅 白宫核心席位</span>
              </div>
              <div className="flex justify-center gap-2 sm:gap-4">
                {core.map(bot => (
                  <PixelPerson key={bot.id} bot={bot} rank="executive"
            ...
            <div className="text-center mb-4">
              <div className="inline-block px-4 py-0.5 rounded-full bg-[#d4a574]/5 border border-[#d4a574]/10 mb-3">
                <span className="text-[10px] sm:text-xs text-[#d4a574]/50 tracking-wider">🏛️ 主要联邦机构</span>
              </div>
              <div className="flex justify-center flex-wrap gap-1.5 sm:gap-3">
                {liubu.map(bot => (
                  <PixelPerson key={bot.id} bot={bot} rank="agency"
                    selected={selectedBot === bot.id} online={bot.hasToken}
                    onClick={() => setSelectedBot(selectedBot === bot.id ? null : bot.id)} />
                ))}
              </div>
            ...
            <div className="text-center">
              <div className="inline-block px-4 py-0.5 rounded-full bg-[#d4a574]/5 border border-[#d4a574]/10 mb-3">
                <span className="text-[10px] sm:text-xs text-[#d4a574]/40 tracking-wider">📚 支撑办公室</span>
              </div>
              <div className="flex justify-center flex-wrap gap-1.5 sm:gap-3">
                {others.map(bot => (
                  <PixelPerson key={bot.id} bot={bot} rank="support"
                    selected={selectedBot === bot.id} online={bot.hasToken}
                    onClick={() => setSelectedBot(selectedBot === bot.id ? null : bot.id)} />
                ))}
              </div>
      ...
      <div className={`${bg} rounded-lg p-3 sm:p-4`}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-xs sm:text-sm font-medium text-[#d4a574]">
            {selectedBot ? `下达指令 → ${bots.find(b => b.id === selectedBot)?.displayName}` : '向白宫战情室发送指令'}
          </h3>
          {selectedBot && (
      ...
      <div className={`${bg} rounded-lg p-3 sm:p-4`}>
        <div className="flex items-center justify-between mb-3">
          <h3 className={`text-[10px] sm:text-xs uppercase tracking-wider ${sub}`}>💬 战情室最新消息</h3>
          <button
